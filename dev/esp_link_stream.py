        ...
        while True:
            try:
                ...
                try:
                    ...
                    data = {'id': esp_id, 'rssi': int(data[1]), 'bat': int(float(data[2])), 'rate': self.rates[esp_id], 'ip': ip, 't_utc': timeutil.timestamp(ms=True), 't': float(data[3]) / 1000.0, 'mag': float(data[4])}
...
if __name__ == "__main__":
    def message_handler(response):
        log.info("\t\t[ID %s] [IP %s] [T %.2f] [RSSI %02d] [BAT %02d] [HZ %02d] [MAG %.2f]" % (response['id'], response['ip'], response['t'], response['rssi'], response['bat'], response['rate'], response['mag']))
